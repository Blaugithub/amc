...
var setTwinValue = function (property) {
    var type;
    for (var i = 0; i < twinArray.length; i++)
        if (twinArray[i].name == property.name) {
            twinArray[i].value = property.value;
            type = twinArray[i].type;
        }
    switch (type) {
        case 'tag':

            var registry = iothub.Registry.fromConnectionString(dev.hubcs);

            var tag = {};
            tag[property.name] = property.value;

            registry.getTwin(dev.deviceId, function (err, twin) {
                if (err) {
            ...
            break;
        case 'property':
            var client = Client.fromConnectionString(dev.devcs, Protocol);
